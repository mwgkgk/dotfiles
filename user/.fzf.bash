...
if [[ ! "$PATH" =~ "/home/mitnick/code/other-src/fzf/bin" ]]; then
  export PATH="/home/mitnick/code/other-src/fzf/bin:$PATH"
fi

# Auto-completion
# ---------------
[[ $- =~ i ]] && source "/home/mitnick/code/other-src/fzf/fzf-completion.bash"
...
__fv() {
  local fpath
  fpath=$(command find -L ${1:-.} \( -path '*/\.*' -o -fstype 'dev' -o -fstype 'proc' \) -prune \
    -o -type d -print -o -type f -print  2> /dev/null | sed 1d | cut -b3- | fzf +m) && printf 'v %q' "$fpath"
}
...
if [ -z "$(set -o | \grep '^vi.*on')" ]; then
  ...
  # ALT-v - open in vim
  bind '"\ev": " \C-e\C-u$(__fv)\e\C-e\er\C-m"'
else
